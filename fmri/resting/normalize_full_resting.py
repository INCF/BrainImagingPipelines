...
def func_datagrabber(name="resting_output_datagrabber"):
    ...
    datasource = pe.Node(interface=nio.DataGrabber(infields=['subject_id','fwhm'],
                                                   outfields=['output','meanfunc','fsl_mat']),
                         name = name)
    datasource.inputs.base_directory = os.path.join(c.sink_dir,'analyses','func')
    datasource.inputs.template ='*'
    datasource.inputs.field_template = dict(output='%s/preproc/output/fwhm_%s/*.nii.gz',
...
def normalize_workflow(name="normalize"):
    ...
    norm.connect(fssource,('aparc_aseg',pickfirst),inputspec,'segmentation')
    ...
    norm.connect(outputspec,'affine_transformation',sinkd,'normalize.affine_transformation')
    norm.connect(outputspec, 'inverse_warp', sinkd, 'normalize.inverse_warp')
    norm.connect(outputspec,'unwarped_brain',sinkd,'normalize.unwarped_brain')
