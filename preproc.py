...
#runs recon-all on those who need it
"""
def run_recon(subj, struct_file, func_files):
    from glob import glob
    import sys
    import os
    ...
    status = ""
    subjid = subj
    #this doesn't actually guarantee a recon is really running, it only prevents the script from exiting
    is_running = lambda s:pjoin(surf_dir, s, "scripts","IsRunning*")
    if glob(is_running(subj)):
        return func_files
    status_log = lambda s:pjoin(surf_dir, s, "scripts", "recon-all-status.log")
    if not os.path.exists(status_log(subj)):
        print "no log found for %s"%subj
    else:
        try:
    ...
    if "finished" in status :
        return func_files
    if not glob(pjoin(surf_dir,subj)):
        cmd0 = 'mkdir '+pjoin(surf_dir,subj)+'; mkdir '+pjoin(surf_dir,subj,'mri')
        cli = CommandLine(command=cmd0)
        cli.run()
    cmd1 = 'mri_convert -i '+struct_file+' -o '+pjoin(surf_dir,subj,'mri','001.mgz')+'; source /software/freesurfer/SetUpFreeSurfer.sh; recon-all -sd '+surf_dir+' -s '+subj+' -all'
    cli = CommandLine(command=cmd1)
    cli.run()
    return func_files
"""
...
def create_prep(name='preproc'):
    ...
    #runs the recon-all node
    
    """run_recon = pe.MapNode(util.Function(input_names=['subj','struct_file','func_files'],
                                         #this just guarantees all the recons are run before continuing; maybe unnecessary
                                         output_names=['func_files'],
                           function=run_recon),
                           name='run_recon')"""
...
#    datasource.inputs.template_args = dict(func=[['subject_id',['3']]])#,'2','3','4','5','6']]])
    ...
    # make connections
    """
    modelflow.connect(dataflow,'infosource.subject_id',             run_recon,  'subj')
    modelflow.connect(dataflow,'datasource.struct',                 run_recon,  'struct_file')
    modelflow.connect(dataflow,'datasource.func',                   run_recon,  'func_files')
    modelflow.connect(run_recon,'func_files',                       preproc,    'inputspec.func')
    """
...
if __name__ == "__main__":
    ...
    #preprocess.run()
    a = create_prep()
    a.base_dir = os.path.join(root_dir,'work_dir')
    a.write_graph()
