...
class InterfaceHelpWriter(object):
    ...
    def generate_api_doc(self, uri):
        ...
        for c in classes:
            ...
            if not issubclass(classinst, HasTraits):
                continue
...
def help(cls, returnhelp=False):
    ...
    allhelp = '\n'.join(docstring + _inputs_help(cls) + [''])
    if returnhelp:
        return allhelp
    else:
        print allhelp
...
def _inputs_help(cls):
    ...
    for name, spec in sorted(inputs.traits(mandatory=True).items()):
        manhelpstr += _get_trait_desc(inputs, name, spec)

    opthelpstr = ['', '\t[Optional]']
    for name, spec in sorted(inputs.traits(mandatory=None,
                                           transient=None).items()):
        opthelpstr += _get_trait_desc(inputs, name, spec)
...
def _get_trait_desc(inputs, name, spec):
        ...
        try:
            ...
            pass
            #manhelpstr+="Something went wrong here"
        except TraitError as excp:
            def_val = ''
            if getattr(spec, 'usedefault'):
                def_val = ', bips default value: %s' % str(getattr(spec, 'default_value')()[1])
