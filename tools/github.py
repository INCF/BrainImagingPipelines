...
def is_git_repo():
    """Does the current bips module have a git folder
    """
    sourcepath = os.path.realpath(os.path.join(os.path.dirname(bips.__file__),
                                               os.path.pardir))
...
def get_local_branch():
    ...
    if is_git_repo():
        o, _ = Popen('git branch | grep "\* "', shell=True, stdout=PIPE,
                     cwd=os.path.dirname(bips.__file__)).communicate()
        return o.strip()[2:]
    else:
        return None
